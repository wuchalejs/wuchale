...
- 9435e25: Always use physical files, change `catalog` config to `localesDir`
...
- b6ea2db: Improve base heuristic to avoid ALLCAPS like POST and EUR, with no lowercase like "M 0,0" in script and attributes
- 8db4eee: Removed the `init` CLI command. Loaders are now specified in the config. And they have to export `getRuntime` and `getRuntimeRx`.
...
- 11e3825: Provide locale as an rgument option for patterns for easy l10n
...
- f893157: Run a pre scan and compile catalogs before build to prevent race conditions
